...
static int check_load_type(uint64 *file_len, char *file_name, int bulen) {
    ...
    if (stat64(file_name, &statbuf) == -1) {
        CLIT_PF("can not get info of %s", file_name);
        return -1;
    }
    *file_len = statbuf.st_size;
    if (bulen < statbuf.st_size) return 0x01;
    return 0x00;
}
