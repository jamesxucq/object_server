...
#include "tran_gcc_cpp.h"
#include "../../debug_funct_sh.h"

//

static int tran_lev1_cont(clit_resu *resul, char *lev1_cont, char *lv2e_cont, osev_clie *oclie) {
    // CLIT_PF("[FUNC]:tran_lev1_cont\n");
    // CLIT_PF("lev1_cont:|%s|\n", lev1_cont);
    ...
    //
    if (trans_lev1_conte(clang_cont, &clsmt, lv2expt/*, oclie->clie_name*/)) {
        free_lv2exp(lv2expt);
        RETU_DELM(clsmt, -1)
    }
    filli_lev1_conte(resul, clang_cont, clsmt.lv2pn, oclie->clie_name);
    // CLIT_PF("End, tran_lev1_cont\n");
    free_lv2exp(lv2expt);
    CLAM_STRU_DELE(clsmt);
    return 0x00;
}

static int tran_lv2p_cont(clit_resu *resul, char *lv2p_cont, char *lv2e_cont, osev_clie *oclie) {
    CLIT_PF("[FUNC]:tran_lv2p_cont\n");
    CLIT_PF("lv2p_cont:|%s|\n", lv2p_cont);
    // CLIT_PF("lv2e_cont:|%s|\n", lv2e_cont);
    char clang_cont[CLANG_CONTE_SIZE];
    MK_ZERO(clang_cont);
    ...
    // CLIT_PF("a1\n");
    if (!trans_lv2p_conte(clang_cont, &clsmt, lv2expt)) {
        free_lv2exp(lv2expt);
        RETU_DELM(clsmt, -1);
    }
    // CLIT_PF("a2\n");
...
// lv2p
// odb-lv2p> kits:list(lv2pn) | kits:translate(GCC);
// odb-lv2p> kits:list(lv2pn, LEVO) | kits:translate(GCC);
// odb-lv2p> kits:list(lv2pn, LVTP) | kits:translate(GCC) | kits:save(txxt.h);
// odb-lv2p> kits:list(lv2pn) | kits:translate(GCC);

// 0x00:OK -1:error

int trans_gcc_cpp(char **pamvp, clit_resu *resul, osev_clie *oclie) {
    CLIT_PF("[FUNC]:trans_gcc_cpp\n");
    CLIT_PF("resul->resu_type:%d comp_type:|%s| basen:|%s|\n", resul->resu_type, pamvp[0x00], oclie->clie_name);
    char conte_data[OSLAN_CONTE_SIZE];
    if (!load_conte_plain(conte_data, OSLAN_CONTE_SIZE - 0x01, resul))
        return -1;
    CLIT_PF("conte_data:|%s|\n", conte_data);
    ...
    CLIT_PF("conte_type:%08X\n", conte_type);
    if (!conte_type) return -1;
    // CLIT_PF("2\n");
    if (KLST_LEVO_CONTE & conte_type) {
        if (tran_lev1_cont(resul, lev1_cont, lv2e_cont, oclie))
            return -1;
    }
    if (KLST_LVTP_CONTE & conte_type) {
        if (tran_lv2p_cont(resul, lv2p_cont, lv2e_cont, oclie))
            return -1;
    }
    //
    return 0x00;
