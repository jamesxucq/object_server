...
static inline int trans_head_norm(char *line_oivk) {
...
static char *tran_class_meth(char *clang_str, char *meth_decl, int norma_const) {
    parse_metho metho;
    INIT_PARSE_METHO(metho)
            // CLIT_PF("meth_decl:|%s|\n", meth_decl);
    if (parse_meth_decla(&metho, meth_decl))
        return NULL;
    if (trans_head_norm(metho.line_oivk))
        return NULL;
...
static char *tran_elv2p_conte(char *clang_str, class_meta *clsmt, lv2p_expa_t *lv2expt) {
    // CLIT_PF("[FUNC]:tran_elv2p_conte\n");
    ...
    for (; menod; menod = menod->next) {
        ...
        clang_str = tran_class_meth(clang_str, menod->lev1_cont, norma_const);
        if (!clang_str) return NULL;
    }
    //
    return clang_str;
...
char *trans_lv2p_conte(char *clang_cont, class_meta *clsmt, lv2p_expa_t *lv2expt) {
    ...
    clang_cont = tran_elv2p_conte(clang_cont, clsmt, lv2expt);
    if (!clang_cont) return NULL;
    //
    clang_cont = FILLI_ELVTP_TAIL(clang_cont);
    // CLIT_PF("end trans_lv2p_conte:\n%s\n", clang_cont);
    return clang_cont;
}
...
static int trans_bind_parm_cons(char *lev1_body, char (*lv2p_prop)[PARAM_LENGTH]) {
    ...
    while (last_str && '\0' != last_str[0x00]) {
        ...
        // CLIT_PF("node_type:%d node_str:|%s|\n", node_type, node_str);
        if (!node_type) {
            clasp = lv2p_prop;
            for (; '\0' != (*clasp)[0x00]; ++clasp) {
                if (!strcmp(node_str, clasp[0x00])) {
                    ...
                    // CLIT_PF("node_str:|%s| clasp[inde]:|%s|\n", node_str, clasp[inde]);
                    break;
                }
            }
        }
...
static int trans_body_cons(char *lev1_body, char (*lv2p_prop)[PARAM_LENGTH]) {
    // CLIT_PF("lev1_body:|%s|\n", lev1_body);
...
static char *trans_metho_const(char *clang_str, char *lev1_cont, char *lv2_dec) {
    ...
    if (trans_head_norm(metho.line_oivk))
        return NULL;
    if (trans_body_cons(metho.lev1_body, lv2p_prop))
        return NULL;
    //
        ...
        if (!node_type) {
            ...
            for (; '\0' != (*formp)[0x00]; ++formp, ++actup) {
                if (!strcmp(node_str, formp[0x00])) {
                    ...
                    // CLIT_PF("node_str:|%s| actu_bind[inde]:|%s|\n", node_str, actu_bind[inde]);
                    break;
                }
            }
        }
...
static int trans_body_norm(char *lev1_body, char form_bind[PARAM_COUNT][PARAM_LENGTH], char actu_bind[PARAM_COUNT][PARAM_LENGTH]) {
    // CLIT_PF("lev1_body:|%s|\n", lev1_body);
...
static char *trans_metho_norma(char *clang_str, char *lev1_cont, char *lv2_dec) {
    ...
    if (trans_head_norm(metho.line_oivk))
        return NULL;
    if (trans_body_norm(metho.lev1_body, form_bind, actu_bind))
        return NULL;
...
static char *tran_meth_data(char *clang_cont, char *lev1_cont, char *lv2_dec, int norma_const) {
    // OSEVPF("[FUNC]:tran_meth_data\n");
    // OSEVPF("lev1_cont:|%s|\n", lev1_cont);
    // OSEVPF("lv2_dec:|%s|\n", lv2_dec);
    ...
    // -1:error 0x00:constructor 0x01:normal
    if (norma_const) {
        last_str = trans_metho_norma(clang_cont, lev1_cont, lv2_dec);
        if (!last_str) return NULL;
    } else {
        last_str = trans_metho_const(clang_cont, lev1_cont, lv2_dec);
        if (!last_str) return NULL;
    }
...
int trans_lev1_conte(char *clang_cont, class_meta *clsmt, lv2p_expa_t *lv2expt/*, char *basen*/) {
    ...
    for (; menod; menod = menod->next) {
        ...
        clang_cont = tran_meth_data(clang_cont, menod->lev1_cont, clsmt->lv2p_decl, norma_const);
        if (!clang_cont) return -1;
    }
    //
    return 0x00;
