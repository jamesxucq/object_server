...
uint32 clit_runni_type(clit_inst *cinst, uint32 cli_mode) {
    ...
    if (CITP_CLEV == cli_mode) {
        ...
        if (INST_CONNE == cinst->inst_levtyp) {
            // CLIT_PF("clit_runni_type conne_type:%d\n", _conne_type_);
            if (OSEV_CONNE == _conne_type_) clie_valu = CITP_OSVB;
...
static int check_level_colle(const char *coll_valu) {
    // CLIT_PF("[FUNC]:check_level_colle, coll_valu:|%s|\n", coll_valu);
    osv_oid_t *coll_obid;
    ...
    // CLIT_PF("check_level_conta, obje_iden:|%s|\n", _leve_defa_obje_.obje_iden);
    return 0x01;
}

static int check_level_conta(const char *cont_iden) {
    // CLIT_PF("[FUNC]:check_level_conta, cont_iden:|%s|\n", cont_iden);
    osv_oid_t *cont_obid;
...
static uint32 run_level_exte(uint32 last_level, const char *clie_valu) {
    // CLIT_PF("[FUNC]:run_level_exte, last_level:%08X clie_valu:|%s|\n", last_level, clie_valu);
    ...
    // CLIT_PF("run_level_exte, run_level:%08X\n", run_level);
    return run_level;
}

uint32 clit_runni_level(clit_inst *cinst, uint32 last_level) {
    uint32 leve_valu = last_level;
    // CLIT_PF("[FUNC]:cinst->inst_levtyp:%u\n", cinst->inst_levtyp);
    ...
    // CLIT_PF("[FUNC]clit_runni_level leve_valu:%d param:|%s|\n", leve_valu, cinst->pamcon[0x00]);
    return leve_valu;
}