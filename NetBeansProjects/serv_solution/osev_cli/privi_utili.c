...
static char *leve_alph(char *alph_valu, char *leve_stri) {
    // CLIT_PF("leve_stri:|%s|\n", leve_stri);
    if (isdigit(leve_stri[0x00])) alph_valu[0x00] = leve_stri[0x00];
    else return NULL;
    if (isalpha((leve_stri + 0x01)[0x00])) {
        alph_valu[0x01] = (++leve_stri)[0x00];
...
uint32 gengr_level_numb(char *leve_stri) {
    // CLIT_PF("gengr_level_numb leve_stri:|%s|\n", leve_stri);
    char alph_stri[0x03];
...
uint32 parse_disti_scope(OUT uint32 *lev_valu, char *dist_iden, const char *objide) {
    CLIT_PF("[FUNC]:parse_disti_scope\n");
    CLIT_PF("dist_iden:|%s| objide:|%s|\n", dist_iden, objide);
    ...
    if (!strncmp(OSEV_DEFAULT_TOK.data, objide, OSEV_DEFAULT_TOK.slen)) {
        ...
        switch (split_disti(basen, levcon, dist_iden)) {
            ...
            case BASE_LEVE: // "odb:2p"
                ...
                lev_valu[0x00] = gengr_level_numb(levcon);
...
// osev: odb:privile:role(GRANT, "develop", ALL_PRIVILE);

uint32 parse_base_privi(char *privi) {
    CLIT_PF("[FUNC]:parse_base_privi\n");
    // CLIT_PF("privi:|%s|\n", privi);
    uint32 priv_valu = INVA_PRIVI;
    char priv_stri[PARAM_LENGTH];
    //
