...
pmmap64 *pmmap_file64(FILE *filptr, unsigned long long mmap_size, unsigned int read_size, unsigned int blk_size) {
    pmmap64 *mmap;
    //
    if (!(mmap = malloc(sizeof (pmmap64)))) {
        fprintf(stderr, "mmap file malloc error\n");
        return NULL;
    }
    memset(mmap, '\0', sizeof (pmmap64));
    //
...
unsigned char *pmmap_ptr64(pmmap64 *mmap, unsigned long long offset, unsigned int msize) {
    unsigned int realen;
    unsigned long long wind_start, read_start;
    unsigned int wind_size, read_size, read_offset;
    //
    if (msize <= 0) {
        fprintf(stderr, "invalid msize passed to mmap_ptr: %ld\n", (long) msize);
        return NULL;
    }
    //
    ...
    if (wind_size > mmap->p_size) {
        ...
        if (!mmap->conptr) fprintf(stderr, "mmap_ptr newc error\n");
    ...
    //
    if (read_size <= 0) {
        fprintf(stderr, "invalid read_size of %ld in mmap_ptr\n", (long) read_size);
        return NULL;
    }
    if (mmap->p_fd_offset != read_start) {
        if (fseeko64(mmap->filptr, read_start, SEEK_SET)) {
    ...
    //
    while (read_size > 0) {
        realen = fread(mmap->conptr + read_offset, 0x01, read_size, mmap->filptr);
        if (realen <= 0) {
