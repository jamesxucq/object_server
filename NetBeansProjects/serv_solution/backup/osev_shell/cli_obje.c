...
int conne_cont_noau(osev_clie *oclie, const char *basen) {
    ...
    CLIEPF("[FUNC]:------ noauth connect to %s.\n", basen);
    // CLIEPF("connect to %s ...\n", conne_str);
    if (reconne_cont(oclie, &clus_auth))
        return -1;
...
osv_oid_t *find_conta_obid(const char *contn, osev_clie *oclie) {
    CLIEPF("[FUNC]:find_conta_obid, start. contn:|%s|\n", contn);
    obje_valu *ovalu;
 ...
 CLIEPF("3 xxxxxxxxxxxxx\n");
    ...
    CLIEPF("end find_conta_obid, echo %s obid:|%s|\n", contn, oidstr(obid_stri, &ovalu->obje_obid));
#endif
...
osv_oid_t *find_base_obid(char *basen, osev_clie *oclie) {
    CLIEPF("[FUNC]:find_base_obid, start. basen:|%s|\n", basen);
    obje_valu *ovalu;
    ...
    if (osev_leve_inte(&stat_resp, OSEV_STATU, OSTAT_ECHO, oclie, &oclie->clie_obid, parms)) {
        OSEVPrintErro("statu echo error!", stat_resp.status_code);
...
#ifdef _DEBUG
    char obid_stri[OBJE_IDENTI_ZLEN];
    CLIEPF("end find_base_obid, echo %s obid:|%s|\n", basen, oidstr(obid_stri, &ovalu->obje_obid));
#endif
    return &ovalu->obje_obid;
}
//
...
void erase_base(const char *basen, const char *objide, unsigned int inst_valu) {
    ...
    if ((CMDI_CLOSE == inst_valu) && !strcmp(OSEV_DEFAULT_VALUE, objide)) {
        strcpy(base_iden, basen);
        // quote_decode(base_iden);
        CLIEPF("erase_conta, base %s objide:|%s|\n", basen, base_iden);
        pthread_mutex_lock(&_base_cache_mutex_);
...
osv_oid_t *find_colle_obid(const char *colle, osev_clie *oclie) {
    CLIEPF("[FUNC]:find_colle_obid, start. colle:|%s|\n", colle);
    obje_valu * ovalu;
    ...
    if (lv2p_colle_list(&invok_resp, oclie, parms)) {
...
#ifdef _DEBUG
    char obid_stri[OBJE_IDENTI_ZLEN];
    CLIEPF("end find_colle_obid, echo %s obid:|%s|\n", colle, oidstr(obid_stri, &ovalu->obje_obid));
#endif
    return &ovalu->obje_obid;
}
