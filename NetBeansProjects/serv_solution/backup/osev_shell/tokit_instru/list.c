...
// level-2+> kits:list(lv2pn);
// level-2+> kits:list(lv2pn, LEVO);
// level-2+> kits:list(lv2pn, LEVO, FORCED);
// level-2+> kits:list(lv2pn, LVTP);
// level-2+> kits:list(lv2pn, LVTP, FORCED);
// level-2+> kits:list(txxt, LVTP) | kits:format(XML);
// 0x00:OK -1:error
...
int kits_list(char **pamvp, clie_resu *resul, osev_clie *oclie) {
    CLIEPF("[FUNC]:kits_list.\n");
    uint64 colen = 0x00;
    ssize_t revlen;
    ...
    // CLIEPF("pamvp[0x00]:|%s| pamvp[0x01]:|%s|\n", pamvp[0x00], pamvp[0x01]);
    //
    ...
    if (-1 == (revlen = requ_lv2p_extint(&lv2p_extern, &lv2p_integr, leve_conte, pamvp[0x00], oclie)))
        return -1;
    CLIEPF("lv2p_extern:|%d| lv2p_integr:|%d|\n", lv2p_extern, lv2p_integr);
    if ((!lv2p_extern || !lv2p_integr) && (!(pamvp[0x01] && pamvp[0x02]) || strcmp(LIST_INTERN_FORCED, pamvp[0x02]))) {
        CliePrintErro("This lv2p is internal or not integral, use FORCED!", 0x00);
        return -1;
    }
    //
    ...
    CLIEPF("kits_list, resul->resu_leng:%llu\n", resul->resu_leng);
    return 0x00;
}
