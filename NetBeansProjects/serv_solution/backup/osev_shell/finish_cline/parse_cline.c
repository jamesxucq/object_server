...
static int cline_leve1(char *line_str) {
    ...
    CLIEPF("2 continue_step:%d continue_type:%d\n", continue_step, continue_type);
...
static int level_check_line(char *line_str, uint32 run_level) {
    ...
    // CLIEPF("[FUNC]level_check_line run_level:%08X line_str:|%s|\n", run_level, line_str);
    ...
    //CLIEPF("[FUNC]level_check_line conti_valu:%d\n", conti_valu);
    return conti_valu;
}
...
static int clie_check_line(char *line_str) {
    ...
    //
    if (!continue_step) {
        // CLIEPF("[FUNC]clie_check_line 1:%d\n", continue_type);
        if (!accep_clie_metho(line_str, objtit_tab[0x00])) {
            continue_step = 0x01;
            // CLIEPF("[FUNC]clie_check_line 2:%d\n", continue_type);
        } else continue_type = 0x00;
    }
    if (0x01 == continue_step) {
        // CLIEPF("[FUNC]clie_check_line 3:%d\n", continue_type);
        continue_type = invok_metho_finis(line_str);
    }
    // CLIEPF("[FUNC]continue_type:%d\n", continue_type);
    if (0x01 != continue_type) continue_step = 0x00;
    return continue_type;
}
...
static int osev_check_line(char *line_str) {
    ...
    //
    if (!continue_step) {
        // CLIEPF("[FUNC]osev_check_line 1\n");
        if (!accep_osev_metho(line_str, objtit_tab[0x00], objtit_tab[0x01]) ||
                !accep_user_metho(line_str)) {
            continue_step = 0x01;
            // CLIEPF("[FUNC]osev_check_line 2\n");
        } else continue_type = -1;
    }
    if (0x01 == continue_step) {
        // CLIEPF("[FUNC]osev_check_line 3\n");
        continue_type = invok_metho_finis(line_str);
    }
    // CLIEPF("[FUNC]continue_type:%d\n", continue_type);
    if (0x01 != continue_type) continue_step = 0x00;
...
int finis_cline_valid(char *line_str, uint32 cli_mode, uint32 run_level) {
    ...
    // CLIEPF("[FUNC]finis_cline_valid conti_valu:%d\n", conti_valu);
    return conti_valu;
}
