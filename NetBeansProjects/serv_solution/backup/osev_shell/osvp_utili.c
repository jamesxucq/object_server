...
int get_conne_type(char *conne_str) {
    conne_str = strstr(conne_str, OSVP_HEAD);
    if (!conne_str) return NULL_CONNE;
    //
    conne_str = strchr(conne_str + sslen(OSVP_HEAD), '/');
    if (!conne_str) return OSEV_CONNE;
    for (++conne_str; '\0' != conne_str[0x00]; ++conne_str) {
        if (' ' != conne_str[0x00]) break;
    }
    char *tokep = strchr(conne_str, '?');
    if (tokep) {
        if (tokep[0x00] == conne_str[0x00] || !strncmp(OSEV_DEFAULT_VALUE, conne_str, sslen(OSEV_DEFAULT_VALUE)))
            return OSEV_CONNE;
    } else {
        if ('\0' == conne_str[0x00] || !strncmp(OSEV_DEFAULT_VALUE, conne_str, sslen(OSEV_DEFAULT_VALUE)))
            return OSEV_CONNE;
    }
    //
    return BASE_CONNE;
...
static int parse_ospuri(serv_addr *saddr, auth_meta *authm, char *osev_base, const char *osvp_uri) {
    ...
    if (tokep && tokde) {
        ...
        tokde[0x00] = '\0';
        tokep[0x00] = '\0';
        parse_host(saddr, toksp);
        split_base(osev_base, tokde + 0x01);
...
int parse_conne_ospuri(osvp_auth *clie_auth, const char *osvp_uri) {
    CLIEPF("[FUNC]:parse_conne_ospuri\n");
    ...
    //
#ifdef _DEBUG
    PRIN_clie_auth(clie_auth, osvp_uri);
#endif
    return parval;
}
