...
char *filli_creat_eobje_exte(char *cont_iden, char *comd_meth, char *line_str) {
    ...
    if (!analy_const_eobje_exte(parms_cont, line_str))
        return NULL;
    // CLIEPF("[FUNC]:filli_creat_eobje_exte, FIX ME!\n");
    ...
    // CLIEPF("[FUNC]:echo_creat_eobje, line_str:|%s|\n", line_str);
    line_str = lccpy(line_str, '(');
...
char *filli_creat_cobje(char *objide, char *comd_meth, char *line_str) {
    ...
    // CLIEPF("[FUNC]:filli_creat_cobje, redi_obje:|%s| parms_cont:|%s|\n", redi_obje, parms_cont);
    line_str = lccpy(line_str, ':');
    char *echo_tok = line_str = lscpy(line_str, OBJE_CREAT_TOK);
    // CLIEPF("const_str:|%s|\n", const_str);
    line_str = lccpy(line_str, '(');
...
char *filli_creat_metho_exte(char *objide, char *comd_meth, char *line_str) {
    ...
    line_str = lccpy(line_str, ':');
    char *echo_tok = line_str = lscpy(line_str, OBJE_CREAT_TOK);
    // CLIEPF("[FUNC]:filli_creat_metho, line_str:|%s|\n", line_str);
    line_str = lscpy(line_str, "(\"");
