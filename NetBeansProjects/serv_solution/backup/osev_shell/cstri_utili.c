...
int inst_lvikoi_type(char *space) {
    // CLIEPF("[FUNC]inst_lvikoi_type space:|%s|\n", space);
    char *toksp, *tokep;
    //
...
char *decod_objide_levivk(char *objide, char *levivk, char *invok, char *coline) {
    char space_conte[OBJN_LENGTH + LVON_LENGTH];
    char *pase_tok = build_space_conte(space_conte, coline);
    // CLIEPF("space_conte:|%s| stri:|%s|\n", space_conte, stri);
...
char *split_levivk_space(char *levivk, char *space) {
    // CLIEPF("[FUNC] split_levivk_space space:|%s|\n", space);
    char *toksp, *tokep;
    //
...
int accep_base_metho(char *line_str, const char *clie_obje, const char *base_obje, const char *kits_obje) {
    char *lstok = pass_blank(line_str);
    if (!lstok) return -1;
    CLIEPF("[FUNC]is_level_method lstok:|%s|\n", lstok);
    if (!strncasecmp(clie_obje, lstok, strlen(clie_obje))) {
        lstok += strlen(clie_obje);
...
int invok_metho_finis(char *line_str) {
    static int end_step;
    // CLIEPF("[FUNC]invok_metho_finis line_str:|%s|\n", line_str);
    // CLIEPF("end_step:%d\n", end_step);
    char *lstok = line_str;
    if (!end_step) {
        lstok = strchr(++lstok, '(');
        // CLIEPF("0 [FUNC]invok_metho_finis lstok:|%s|\n", lstok);
        if (lstok) end_step = 0x01;
    }
    if (0x01 == end_step) {
        lstok = pass_parms(++lstok);
        // CLIEPF("1 [FUNC]invok_metho_finis lstok:|%s|\n", lstok);
        if (lstok) end_step = 0x02;
    }
    if (0x02 == end_step) {
        lstok = qstrch(++lstok, ';');
        // CLIEPF("2 [FUNC]invok_metho_finis lstok:|%s|\n", lstok);
        if (lstok) end_step = 0x03;
    }
    if (0x03 == end_step) {
        end_step = 0x00;
        // CLIEPF("3 [FUNC]invok_metho_finis lstok:|%s|\n", lstok);
...
int metho_defin_finis(char *line_str) {
    // CLIEPF("[FUNC]:metho_defin_finis line_str:|%s|\n", line_str);
    static int end_step;
    //
    char *lstok = line_str;
    if (!end_step) {
...
static const char *lv12p_base_comd_tab[] = {"post", "list", "create", "drop", "replace", "integra", "reload", NULL};

#define LEVE_CLIE_TOK "clie"
static const char *lv12p_clie_comd_tab[] = {"level", "quit", NULL};

int creat_class_metho(const char *objide, char *lev1n) {
    char **ptok;
    if (!strcasecmp(LEVE_OBJE_TOK, objide)) {
        ptok = (char **) lv12p_base_comd_tab;
        for (; ptok[0x00]; ++ptok) {
            if (!strcasecmp(lev1n, ptok[0x00]))
                return 0x01;
        }
    }
    if (!strcasecmp(LEVE_CLIE_TOK, objide)) {
        ptok = (char **) lv12p_clie_comd_tab;
        for (; ptok[0x00]; ++ptok) {
            if (!strcasecmp(lev1n, ptok[0x00]))
                return 0x01;
        }
...
void CliePrintWarn(const char *fmt, ...) {
...
static const osv_str_t *exce_objide_tab[] = {&odb_stri, &clie_stri, NULL};

int valid_osev_objide(char *objide) {
    osv_str_t **ptok = (osv_str_t **) exce_objide_tab;
    for (; ptok[0x00]; ++ptok) {
...
void CliePrintErro(char *err_labe, int err_num) {
    if (err_num) fprintf(stderr, CLIEE_PROMPT"%s errno:%d\n", err_labe, err_num);
    else fprintf(stderr, CLIEE_PROMPT"%s\n", err_labe);
}