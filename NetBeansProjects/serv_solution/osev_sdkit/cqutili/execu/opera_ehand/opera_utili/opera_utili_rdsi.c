...
#include "opera_utili_rdsi.h"

// rdsi
//

static int inse_file_hmd_rdsi(OBIDHMD *ohmd, FILE *fild, FILE *data) {
...
int merge_data_obid_rdsi(FILE *filed, FILE *file1, FILE *file2) {
    ...
    if (!ohmd) return -1;
    if (inse_file_hmd_rdsi(ohmd, filed, file2)) {
...
static int inse_data_hmd_rdsi(OBIDHMD *ohmd, FILE *data) {
    OSEVPF("[FUNC]:inse_data_db\n");
    osv_oid_i reso_obid;
    rewind(data);
    while (0x00 < fread(&reso_obid, sizeof (osv_oid_i), 0x01, data)) {
...
int shared_data_obid_rdsi(FILE *filed, FILE *file1, FILE *file2) {
    ...
    OBIDHMD *ohmd = creat_temp_ohmd();
    if (!ohmd) return -1;
    if (inse_data_hmd_rdsi(ohmd, file2)) {
        oidhmd_close(ohmd, 0x00);
        oidhmd_dele(ohmd);
        return -1;
    }
