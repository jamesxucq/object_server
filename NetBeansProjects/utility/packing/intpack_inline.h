...
#define PK_SIZE_CHECK_PACK(l, maxl) PK_RET_TEST((maxl) != 0 && (size_t)(l) > (maxl), ENOMEM)
#define PK_SIZE_CHECK_UNPACK(l, maxl) PK_RET_TEST((maxl) != 0 && (size_t)(l) > (maxl), EINVAL)

// Count the leading zero bytes.
#if defined(__GNUC__)
#define PK_LEADING_ZEROS(x, i) ((i) = ((x) == 0) ? (int)sizeof(x) : __builtin_clzll(x) >> 3)
#elif defined(_MSC_VER)
#define PK_LEADING_ZEROS(x, i)              \
    do {                                    \
        if ((x) == 0)                       \
            (i) = (int)sizeof(x);           \
        else {                              \
...
static inline int __pk_vpack_posint(uint8_t **pp, size_t maxlen, uint64_t x) {
    uint8_t *p;
    int len, lz, shift;

    PK_LEADING_ZEROS(x, lz);
    len = (int) sizeof (x) - lz;
    PK_SIZE_CHECK_PACK(len + 1, maxlen);
    p = *pp;
...
static inline int __pk_vpack_negint(uint8_t **pp, size_t maxlen, uint64_t x) {
    uint8_t *p;
    int len, lz, shift;
...
static inline int __pk_vunpack_posint(const uint8_t **pp, size_t maxlen, uint64_t *retp) {
    uint64_t x;
    uint8_t len;
    const uint8_t *p;
    ...
    p = *pp;
    len = (*p++ & 0xf);
    PK_SIZE_CHECK_UNPACK(len + 1, maxlen);
...
static inline int __pk_vunpack_posint_size(const uint8_t **pp, size_t maxlen, size_t *sizp) {
    uint64_t x;
    uint8_t len;
    const uint8_t *p;
...
static inline int __pk_vunpack_negint(const uint8_t **pp, size_t maxlen, uint64_t *retp) {
    uint64_t x;
    uint8_t len;
    const uint8_t *p;
...
static inline int __pk_vunpack_negint_size(const uint8_t **pp, size_t maxlen, size_t *sizp) {
    uint64_t x;
    uint8_t len;
    const uint8_t *p;
...
static inline int __pk_vpack_uint(uint8_t **pp, size_t maxlen, uint64_t x) {
    uint8_t *p;
    ...
    if (x <= POS_1BYTE_MAX)
        ...
        *p++ = 0;
        ...
        x -= POS_2BYTE_MAX + 1;
        *p = POS_MULTI_MARKER;
        return (__pk_vpack_posint(pp, maxlen, x));
    }
...
static inline int __pk_vpack_int(uint8_t **pp, size_t maxlen, int64_t x) {
    uint8_t *p;
    ...
    if (x < NEG_2BYTE_MIN) {
        *p = NEG_MULTI_MARKER;
        return (__pk_vpack_negint(pp, maxlen, (uint64_t) x));
    }
    if (x < NEG_1BYTE_MIN) {
        PK_SIZE_CHECK_PACK(2, maxlen);
        x -= NEG_2BYTE_MIN;
        ...
        *p++ = NEG_1BYTE_MARKER | GET_BITS(x, 6, 0);
        ...
        return (__pk_vpack_uint(pp, maxlen, (uint64_t) x));
...
static inline int __pk_vunpack_uint(const uint8_t **pp, size_t maxlen, uint64_t *xp) {
    const uint8_t *p;
    ...
    switch (*p & 0xf0) {
        ...
        case POS_2BYTE_MARKER | 0x10:
            PK_SIZE_CHECK_UNPACK(2, maxlen);
            *xp = GET_BITS(*p++, 5, 0) << 8;
            *xp |= *p++;
            *xp += POS_1BYTE_MAX + 1;
            break;
        case POS_MULTI_MARKER:
            PK_RET(__pk_vunpack_posint(pp, maxlen, xp));
            *xp += POS_2BYTE_MAX + 1;
            return (0);
        default:
            return (EINVAL);
...
static inline int __pk_vunpack_int(const uint8_t **pp, size_t maxlen, int64_t *xp) {
    const uint8_t *p;
    ...
    switch (*p & 0xf0) {
        case NEG_MULTI_MARKER:
            PK_RET(__pk_vunpack_negint(pp, maxlen, (uint64_t *) xp));
            return (0);
        ...
        default:
            ...
            return (__pk_vunpack_uint(pp, maxlen, (uint64_t *) xp));
    }
...
static inline int __pk_vunpack_uint_size(const uint8_t **pp, size_t maxlen, size_t *sizp) {
    const uint8_t *p;
    ...
    switch (*p & 0xf0) {
        ...
        case POS_2BYTE_MARKER | 0x10:
            *sizp = 0x02;
            ...
            break;
        case POS_MULTI_MARKER:
            PK_RET(__pk_vunpack_posint_size(pp, maxlen, sizp));
            return (0);
        default:
            return (EINVAL);
    }
...
static inline int __pk_vunpack_int_size(const uint8_t **pp, size_t maxlen, size_t *sizp) {
    const uint8_t *p;
    ...
    switch (*p & 0xf0) {
        case NEG_MULTI_MARKER:
            PK_RET(__pk_vunpack_negint_size(pp, maxlen, sizp));
            return (0);
        case NEG_2BYTE_MARKER:
        case NEG_2BYTE_MARKER | 0x10:
            PK_SIZE_CHECK_UNPACK(2, maxlen);
        ...
        default:
            // Identical to the unsigned case.
            return (__pk_vunpack_uint_size(pp, maxlen, sizp));
    }
...
static inline size_t __pk_vsize_posint(uint64_t x) {
    int lz;

    PK_LEADING_ZEROS(x, lz);
    return ((size_t) (PK_INTPACK64_MAXSIZE - lz));
}

// __pk_vsize_negint --
// Return the packed size of a negative variable-length integer.

static inline size_t __pk_vsize_negint(uint64_t x) {
    int lz;
...
static inline size_t __pk_vsize_uint(uint64_t x) {
    if (x <= POS_1BYTE_MAX)
        return (1);
    if (x <= POS_2BYTE_MAX + 1)
        return (2);
    x -= POS_2BYTE_MAX + 1;
    return (__pk_vsize_posint(x));
}

// __pk_vsize_int --
// Return the packed size of a signed integer.

static inline size_t __pk_vsize_int(int64_t x) {
    if (x < NEG_2BYTE_MIN)
        return (__pk_vsize_negint((uint64_t) x));
    if (x < NEG_1BYTE_MIN)
        return (2);
    if (x < 0)
        return (1);
    // For non-negative values, use the unsigned code above.
    return (__pk_vsize_uint((uint64_t) x));
}
...
static inline int __pk_iunpack_posint(const uint8_t *ip, size_t maxlen, uint64_t *retp) {
    uint64_t x;
    uint8_t len;
    const uint8_t *p;
...
static inline int __pk_iunpack_negint(const uint8_t *ip, size_t maxlen, uint64_t *retp) {
    uint64_t x;
    uint8_t len;
    const uint8_t *p;
...
static inline int __pk_iunpack_uint(const uint8_t *ip, size_t maxlen, uint64_t *xp) {
    const uint8_t *p;
    ...
    switch (*p & 0xf0) {
        ...
        case POS_2BYTE_MARKER | 0x10:
            ...
            *xp += POS_1BYTE_MAX + 1;
            break;
        case POS_MULTI_MARKER:
            PK_RET(__pk_iunpack_posint(ip, maxlen, xp));
            *xp += POS_2BYTE_MAX + 1;
            return (0);
        default:
            return (EINVAL);
...
static inline int __pk_iunpack_int(const uint8_t *ip, size_t maxlen, int64_t *xp) {
    const uint8_t *p;
    ...
    switch (*p & 0xf0) {
        case NEG_MULTI_MARKER:
            PK_RET(__pk_iunpack_negint(ip, maxlen, (uint64_t *) xp));
            return (0);
        case NEG_2BYTE_MARKER:
        case NEG_2BYTE_MARKER | 0x10:
            PK_SIZE_CHECK_UNPACK(2, maxlen);
        ...
        default:
            // Identical to the unsigned case.
            return (__pk_iunpack_uint(ip, maxlen, (uint64_t *) xp));
    }
