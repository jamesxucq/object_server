...
static unsigned long long bulid_invok_pamvs(pamstyl_t *pscon, char *pamcon) {
    printf("[FUNC]:bulid_invok_pamvs\n");
    const char *invok_pamvp[PARAM_COUNT];
    char _iline_[LARGE_CONTE_SIZE];
    if (-1 == split_instru_pamvs(invok_pamvp, _iline_, pamcon))
        return INVA_ULOLO_VALU;
    if (-1 == pretr_functi_pamvs(invok_pamvp))
        return INVA_ULOLO_VALU;
    //
    unsigned long long colen = encod_list_loop(pscon->pamstyl_memo + sizeof (binary_entit), invok_pamvp);
    if (INVA_ULOLO_VALU == colen)
        return INVA_ULOLO_VALU;
    printf("[FUNC]:colen:%llu\n", colen);
...
unsigned long long parse_invok_metho(pamstyl_t *pscon, char *pamcon) {
    ...
    printf("objide:|%s| inst_levivk:|%s| inst_invok:|%s|\n", objide, inst_levivk, inst_invok);
